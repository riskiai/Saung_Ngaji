...
class Siswa extends CI_Controller
{
	...
	public function edit_siswa($id)
	{
		if ($this->input->post('edit_siswa')) {
			...
			$data['detail'] = $this->siswa_model->detail_siswa($id);
			// $x = $this->siswa_model->detail_siswa($id);
			// var_dump ($x); die;
	...
	public function filter_laporan()
	{
		...
		if ($this->input->post('submit')) {
			...
			$data['list_laporan'] = $this->siswa_model->list_laporan($start);
	...
	public function delete_pendaftar($id = 0)
	{
		...
		$this->session->set_flashdata('message', 'Siswa Pendaftar berhasil dihapus!');
		redirect(base_url('admin/siswa/pendaftar'));
	}

	public function delete_siswa($id = 0)
	{
